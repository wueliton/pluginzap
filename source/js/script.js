...
$(document).ready(function() {
	...
	$("#payment").click(function() {
		...
		$.post("responses/planData.php",function(response) {
			if(response.status==true) {
				...
				if(days < 31) {
					days = 30 - days;
					$("#trialExpiration").html('<p class="aviso">Faltam '+days+' dias para seu período de testes acabar.</p><br/>');
				}
				...
				switch (planStatus) {
					case 'ACTIVE':
						...
						break;
					case 'PENDING' :
						planStatus = 'Pagamento em Análise';
						break;
					case 'CANCELLED' :
						planStatus = 'Assinatura Cancelada';
						break;
					case 'PAYMENT_METHOD_CHANGE':
						planStatus = 'Meio de Pagamento Inválido.';
						break;
				}
	...
	$("#themeSelect a.btn-primary").click(function() {
		if($(".theme.active").length) {
			...
			notification("É preciso escolher um tema para continuar.");
		}
	})
	...
	$("body").append("<div class=\"confirmWindow\"><div class=\"content\"><h2>Excluir</h2><p>Tem certeza que deseja excluir <strong>Exemplo</strong>?</p><span class=\"aviso\">Essa ação irá excluir esse contato e todas as mensagens enviadas por ele.</span><br/><br/><div class=\"text-right\"><a href=\"javascript:void(0)\" class=\"close\">Cancelar</a><button class=\"btn-primary danger\">Excluir</a></div></div></div>");
    ...
    PagSeguroDirectPayment.createCardToken({
        ...
        error: function(response) {
			for(var i in response.errors) {
				if (i=="10000") {
					error = "Bandeira de cartão inválida";
					$("#payForm input[name=numeroCartao]").addClass("error");
				}
				else if (i=="10001") {
					error = "Número de cartão de crédito "+ appPayment.paymentMethodOptio.name +" inválido";
					$("#payForm input[name=numeroCartao]").addClass("error");
				}
				else if (i=="10002") {
					...
					error = "Informe o código de segurança";
