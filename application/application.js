...
if(isset($_GET["code"])) {
    ...
    $urlSite = "http://localhost/gerencia-zap/application/send.php";
...
?>
urlBase = "http://localhost/gerencia-zap/uploads/";
...
document.body.innerHTML += '<div id="'+id+'"></div>';
document.querySelector("#"+id).innerHTML='<a href="javascript:void(0)" class="wtsIcon"> <span class="textNtWTS">Chamar no WhatsApp!</span> <i class="notificationWTS">1</i> <img src="'+urlBase+'whatsapp.svg" alt="WhatsApp"> </a> <div class="windowWTS"> <div class="formWTS"> <header> <i class="closeWTS">x</i> <div class="profileWTS"><img src="'+image+'" alt=""><i class="onlineWTS"></i></div><div class="infoWTS"><div>'+empresa+'<span>online</span></div></div></header> <form action="<?=$urlSite?>" method="post" target="_blank"><input type="hidden" name="url" value="'+url_atual+'" /> <input type="hidden" name="idEnterprise" value="<?=$idEnterprise?>" /> <input type="text" name="name" placeholder="Nome" required> <input type="tel" name="whatsapp" placeholder="DDD + Número" required> <input type="email" name="mail" placeholder="E-mail" required> <textarea name="message" rows="3" placeholder="Sua Mensagem" required></textarea> <input type="submit" value="Enviar"> </form> </div></div>';
